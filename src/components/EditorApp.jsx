...
const EditorApp = () => {
  ...
  async function reviewCode() {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/review`, {
        ...
        body: JSON.stringify({ code, language: selectedOption.value }),
      });
      const data = await res.json();
      setResponse(data.analysis || data.feedback || "No feedback received from AI.");
    } catch (err) {
  ...
  async function visualizeCode() {
    ...
    try {
      ...
      console.error("Error visualizing code:", err);
      alert("Error analyzing code. Please check your backend logs.");
    } finally {
      setLoading(false);
    }
      ...
      <div className="main flex justify-between" style={{ height: "calc(100vh - 90px)" }}>
          ...
          <div className="tabs !p-3 w-full flex items-center gap-5">
            <Select
              ...
              styles={{
                ...
                option: (base, state) => ({ ...base, backgroundColor: state.isFocused ? '#27272a' : '#18181b', color: 'white' }),
        ...
        <div className="right !p-[10px] bg-zinc-900 w-[50%] h-[100%] overflow-auto">
          <div className="topTab gap-3 justify-center border-t-[1px] border-b-[1px] border-[#3c3c3d] flex items-center h-[60px]">
            <p className="bg-gradient-to-r from-[#1cacd0] via-[#628d98] to-[#9fa6a7] bg-clip-text text-transparent font-light text-[28px]">
              Response
            </p>
            <AiOutlineDeploymentUnit color="#42a2d2" size={33} />
          </div>
          {loading ? (
